...
async function exportWebPublication({ exportAliases, manifestPath, input, output, }) {
    ...
    try {
        ...
        for (const entry of relExportAliases) {
            ...
            if (util_1.pathEquals(upath_1.default.join(input, entry.source), manifestPath)) {
        ...
        for (const entry of relExportAliases) {
            const rewriteAliasPath = (e) => {
                if (typeof e === 'string') {
                    return util_1.pathEquals(e, entry.source) ? entry.source : e;
                }
                if (util_1.pathEquals(e.url, entry.source)) {
                    e.url = entry.target;
                }
                return e;
            };
