...
const pubManifest_1 = require("./schema/pubManifest");
...
function generateManifest(outputPath, entryContextDir, options) {
    ...
    if (options.cover) {
        ...
        if (!type || !mimeType) {
            util_1.log(`\n${chalk_1.default.yellow('Cover image ')}${chalk_1.default.bold.yellow(`"${options.cover}"`)}${chalk_1.default.yellow(' was set in your configuration but couldn’t detect the image metadata. Please check a valid cover file is placed.')}`);
        }
    }
    const publication = {
    ...
    fs_1.default.writeFileSync(outputPath, publicationJson);
    const ajv = new ajv_1.default({ strict: false });
    ajv_formats_1.default(ajv);
    ajv.addSchema(pubManifest_1.publicationSchemas);
    const validate = ajv.compile(pubManifest_1.publicationSchema);
    ...
    if (!valid) {
        ...
        const detailMessage = validate.errors &&
            better_ajv_errors_1.default(pubManifest_1.publicationSchemas, publication, util_1.filterRelevantAjvErrors(validate.errors), {
...
async function compile({ entryContextDir, workspaceDir, manifestPath, manifestAutoGenerate, themeIndexes, entries, language, readingProgression, cover, vfmOptions, input, }, { reload = false } = {}) {
    ...
    for (const entry of contentEntries) {
        ...
        if (entry.type === 'text/markdown') {
            ...
            if (!util_1.pathEquals(entry.source, entry.target)) {
